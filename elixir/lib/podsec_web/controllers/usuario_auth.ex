defmodule PodsecWeb.UsuarioAuth do
  import Plug.Conn
  import Phoenix.Controller

  alias Podsec.Authentication
  alias PodsecWeb.Router.Helpers, as: Routes
  ...
  def log_out_usuario(conn) do
    ...
    usuario_token && Authentication.delete_session_token(usuario_token)

    if live_socket_id = get_session(conn, :live_socket_id) do
      PodsecWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def fetch_current_usuario(conn, _opts) do
    ...
    usuario = usuario_token && Authentication.get_usuario_by_session_token(usuario_token)
    assign(conn, :current_usuario, usuario)
  end

  defp ensure_usuario_token(conn) do
    if usuario_token = get_session(conn, :usuario_token) do
      {usuario_token, conn}
    else
      conn = fetch_cookies(conn, signed: [@remember_me_cookie])

      if usuario_token = conn.cookies[@remember_me_cookie] do
        {usuario_token, put_session(conn, :usuario_token, usuario_token)}
      else
        {nil, conn}
      end
    end
